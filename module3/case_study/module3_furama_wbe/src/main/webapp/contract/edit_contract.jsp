    ...
    <div class="bg-pink text-white col-5" style="margin: 0 auto !important; padding: 20px;">
        <form method="post">
            ...
            <div class="form-outline mb-1 w-75">
                ...
                <input type="date" id="form1Example2" class="form-control" name="dateCheckOut" value="${dateCheckOut}"/>
            </div>

            <div class="form-outline mb-1 w-75">
                <label class="form-label" for="form1Example3">deposit</label>
                <input type="text" id="form1Example3" class="form-control" name="deposit" value="${deposit}"/>
            </div>

            <div class="form-outline mb-1 w-75">
                <label class="form-label" for="form1Example4">totalMoney</label>
                <input type="text" id="form1Example4" class="form-control" name="totalMoney"  value="${totalMoney}"/>
            </div>

            <div class="form-outline mb-2 w-75">
                <label class="form-label" for="form1Example5">Employee</label>
                <select class="form-select" aria-label="Default select example" id="form1Example5"
                        name="employee">
                    <c:forEach items="${employeeList}" var="i">
                        <c:choose>
                            <c:when test="${i.id == employeeId}">
            ...
            <div class="form-outline mb-2 w-75">
                ...
                <select class="form-select" aria-label="Default select example" id="form1Example6"
                        name="customer">
                    <c:forEach items="${customerList}" var="i">
                        <c:choose>
                            <c:when test="${i.id == customerId}">
                                <option value="${i.id}" selected>${i.name}</option>
                            </c:when>
                            <c:otherwise>
                                <option value="${i.id}">${i.name}</option>
            ...
            <div class="form-outline mb-2 w-75">
                ...
                <select class="form-select" aria-label="Default select example" id="form1Example7"
                        name="service">
                    <c:forEach items="${serviceList}" var="i">
                        <c:choose>
                            <c:when test="${i.id == serviceId}">
                                <option value="${i.id}" selected>${i.name}</option>
                            </c:when>
                            <c:otherwise>
                                <option value="${i.id}">${i.name}</option>
